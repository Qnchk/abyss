  ...
  const filteredQuestions = useMemo(() => {
    return questions.filter((q) => {
      ...
      if (filters.companies.length > 0) {
        const qCompanies = q.companies || [];
        const hasAny = filters.companies.some((c) => qCompanies.includes(c));
        if (!hasAny) return false;
      }
      if (filters.onlyWithCompany && (!q.companies || q.companies.length === 0)) return false;
      ...
      <div className={"layout" + (sidebarCollapsed ? " sidebar-collapsed" : "")}>
        ...
        <aside className={"sidebar" + (sidebarCollapsed ? " collapsed" : "")}>
          <section className="card">
            ...
            <div className="filter-block">
              <div className="filter-label">{t.companies}</div>
              <div className="tag-picker">
                ...
                >
                  {filters.companies.length > 0
                    ? `Выбрано: ${filters.companies.length}`
                    : "Выбрать фонды"}
                  <span className="chevron">{showCompanyPicker ? "▲" : "▼"}</span>
                </button>
                    ...
                    {filters.companies.map((c) => (
                      <button
                        ...
                        onClick={() =>
                          setFilters((prev) => ({
                            ...prev,
                            companies: prev.companies.filter((x) => x !== c),
                          }))
                        }
                      >
                        {c} <span>×</span>
                ...
                {showCompanyPicker && (
                  <div className="tag-picker-panel">
                    {uniqueCompanies.length === 0 && (
                      <div className="tag-picker-empty">Нет фондов</div>
                    )}
                    {uniqueCompanies.map((c) => {
                      const checked = filters.companies.includes(c);
                      return (
                        <label
                          key={c}
            ...
            <label className="checkbox-row">
              <input
                ...
                onChange={(e) =>
                  setFilters((prev) => ({
                    ...prev,
                    onlyWithCompany: e.target.checked,
                  }))
                }
              />
              {t.onlyWithCompany}
